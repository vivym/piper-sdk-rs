...
* **补充**：在随动主从模式下，示教输入臂还会主动发送 `0x151`、`0x155`、`0x156`、`0x157`、`0x159`；其载荷定义与本文对应控制指令完全一致，可作为主臂目标值/控制状态进行接收解析。
...
* **ID 范围**：`0x0A0~0x0AF`, `0x0B0~0x0BF`, `0x4A0~0x4AE`, `0x4B0~0x4BF`, `0x4C0~0x4CF`
* **说明**：上述范围主要用于固件升级相关内部指令。`0x4AF` 已被官方 SDK 公开用作固件版本查询/反馈指令，见本文对应条目。
...
* 当机械臂被配置为示教输入臂时，会主动发送 `0x151`、`0x155`、`0x156`、`0x157`、`0x159`。
* 以上报文的字段定义与第 5 节中对应控制指令完全一致，可按同一格式解析，用于读取主臂发送给从臂的模式、关节目标值和夹爪目标值。
...
* **Byte 6**: 保留
* **Byte 7**: 保留
* *说明：控制模式目前仅对用户开放 0x01 CAN指令控制模式；作为模式切换指令时，Byte2~Byte7全部填充默认值0x0即可。官方 SDK 已支持 `0x05 = MOVE CPV`，但官方材料对最早支持该模式的固件版本存在 `V1.6.5` 与 `V1.8-1` 两种说法。*
...
* 0x03: 终点
* **Byte 1-7**: 填充 0x00
* *说明：该指令的有效载荷仅使用 Byte0；官方 SDK 实际发送时采用 8 字节 CAN 帧，Byte1~Byte7 置 0。*
...
* **Pos_ref**: 16bit，量程 `[-12.5, 12.5]`
* **Vel_ref**: 12bit，量程 `[-45.0, 45.0]`
* **Kp**: 12bit，量程 `[0.0, 500.0]`
* **Kd**: 12bit，量程 `[-5.0, 5.0]`
* **T_ref**: 8bit。官方 SDK 公开接口文档写 `[-18.0, 18.0]`，但官方实际编码实现固定使用 `[-8.0, 8.0]`；该项存在官方材料不一致，使用时建议结合目标固件实测确认。
* **CRC**: 官方 SDK 的编码实现按 `Byte0 ^ Byte1 ^ ... ^ Byte6` 取低 4 bit，填入 Byte7 低 4 bit。
...
* *说明：随动主从模式支持ID偏移值配对的示教输入臂同步控制运动输出臂。1.设置为示教输入臂后，需要重启或者进入待机模式后方可切换为联动示教输入模式，此模式下不响应控制指令，不再主动发送周期反馈报文，且会主动发送 `0x151`、`0x155`、`0x156`、`0x157`、`0x159` 这些控制帧；设置成功会保存到flash，掉电不丢失。2.设置为运动输出臂后恢复为常规状态（退出联动示教输入模式，进入待机模式）；未收到此条指令的机械臂默认为此状态。官方 SDK 实际发送时采用 8 字节 CAN 帧，Byte4~Byte7 置 0。*
...
* **Byte 1**: 转速低八位
* **Byte 2**: 电流高八位 (signed int16, 电机当前电流 单位：0.001A)
* **Byte 3**: 电流低八位
...
### ID: 0x4AF 固件版本查询/反馈指令 (Len: 8)

* **查询方向**：决策控制单元 -> 机械臂主控
* **反馈方向**：机械臂主控 -> 决策控制单元
* **查询帧**：官方 SDK 使用 `01 00 00 00 00 00 00 00`
* **反馈载荷**：固件版本字符串的原始字节流；官方 SDK 会按收到的多帧数据顺序拼接后再解析版本号。
* **版本提取规则（官方 SDK 行为）**：在拼接后的字节流中查找以 `S-V` 开头的 8 字节版本字段。
* *说明：`0x4AF` 已被官方 SDK 公开作为固件版本查询/反馈指令使用，不应再归类为“用户不需要关心”的纯升级内部指令。*
