...
const MAX_TARGET_DELTA_M: f64 = 0.02;
const MAX_VIA_OFFSET_M: f64 = 0.015;
const DEFAULT_SETTLE_TIMEOUT_MS: u64 = 12_000;
const POSITION_SETTLE_TOLERANCE_M: f64 = 0.003;
const MIN_PROGRESS_M: f64 = 0.001;
const MIN_VIA_NEIGHBORHOOD_RADIUS_M: f64 = 0.0015;
...
#[derive(Parser, Debug)]
#[command(name = "hil_circular_motion_check")]
#[command(about = "Safe circular motion HIL helper for one real Piper arm")]
struct Args {
    ...
    /// Signed Y offset for the circular via point, in meters.
...
fn run_circular_check<Capability>(
    ...
    Capability: MotionCapability,
{
    ...
    println!(
        "[PASS] enabled PositionMode motion=Circular speed_percent={}",
        args.speed_percent
    );
    ...
    let via_position = circular_via_position(initial_position, target_position, args.via_offset_m);
    ...
    println!(
        "[PASS] command step=move via_m=({:.6}, {:.6}, {:.6}) target_m=({:.6}, {:.6}, {:.6}) delta_m={:.6} speed_percent={}",
        via_position.x,
        via_position.y,
        via_position.z,
        target_position.x,
    ...
    let moved = wait_for_circular_settle(
        observer,
        initial_position,
        via_position,
        target_position,
    ...
    println!(
        "[PASS] settle step=move observed_delta_m={:.6} target_delta_m={:.6} position_error_m={:.6} via_seen={}",
        observed_delta, target_delta_m, move_error, moved.via_seen
    );

    let return_via_position =
        circular_via_position(target_position, initial_position, -args.via_offset_m);
    robot.move_circular(
        return_via_position,
        target_orientation,
        initial_position,
    ...
    println!(
        "[PASS] command step=return via_m=({:.6}, {:.6}, {:.6}) target_m=({:.6}, {:.6}, {:.6})",
        return_via_position.x,
        return_via_position.y,
        return_via_position.z,
        initial_position.x,
        initial_position.y,
        initial_position.z
    );

    let returned = wait_for_circular_settle(
        observer,
        target_position,
        return_via_position,
        initial_position,
    ...
    println!(
        "[PASS] settle step=return final_position_m=({:.6}, {:.6}, {:.6}) return_error_m={:.6} via_seen={} tolerance_m={:.6}",
        returned_position.x,
        returned_position.y,
        returned_position.z,
        return_error,
    ...
    println!("[PASS] hil_circular_motion_check complete");
...
fn validate_args(args: &Args) -> Result<(), String> {
    ...
    if !args.delta_m.is_finite() || !args.via_offset_m.is_finite() {
        return Err("delta_m and via_offset_m must be finite for manual HIL".to_string());
    }
    if args.delta_m == 0.0 {
        return Err("delta_m must be non-zero for manual HIL".to_string());
    }
    if args.via_offset_m == 0.0 {
        return Err("via_offset_m must be non-zero for manual HIL".to_string());
    }
    if args.delta_m.abs() > MAX_TARGET_DELTA_M {
        return Err("delta_m must be <= 0.02 m for manual HIL".to_string());
    }
    if args.via_offset_m.abs() > MAX_VIA_OFFSET_M {
        return Err("via_offset_m must be <= 0.015 m for manual HIL".to_string());
    }
...
fn circular_via_position(start: Position3D, target: Position3D, via_offset_m: f64) -> Position3D {
    Position3D::new(
        (start.x + target.x) * 0.5,
        (start.y + target.y) * 0.5 + via_offset_m,
        (start.z + target.z) * 0.5,
    )
}

fn within_via_neighborhood(sample: Position3D, via: Position3D, radius_m: f64) -> bool {
    translation_distance(sample, via) <= radius_m
}

fn via_neighborhood_radius(target_delta_m: f64) -> f64 {
    (target_delta_m * 0.25).max(MIN_VIA_NEIGHBORHOOD_RADIUS_M)
}
...
struct CircularSettleResult {
    end_pose: piper_sdk::driver::state::EndPoseState,
    via_seen: bool,
}

fn wait_for_circular_settle<Capability>(
    ...
    timeout: Duration,
    poll_interval: Duration,
) -> ClientResult<CircularSettleResult>
where
    Capability: MotionCapability,
{
    let start = Instant::now();
    ...
    let via_radius = via_neighborhood_radius(total_delta);
    ...
    loop {
        ...
        if !via_seen && within_via_neighborhood(observed_position, via_position, via_radius) {
            via_seen = true;
        }

        if progress_seen && via_seen && remaining_error <= POSITION_SETTLE_TOLERANCE_M {
            return Ok(CircularSettleResult {
                end_pose: pose,
                via_seen,
            });
        }
...
#[test]
fn validate_args_rejects_zero_via_offset() {
    let args = Args {
    ...
    let error = validate_args(&args).expect_err("zero via offset must be rejected");
    assert!(error.contains("via_offset_m"));
}

#[test]
fn sample_within_via_neighborhood_is_detected() {
    let via = Position3D::new(0.01, 0.005, 0.0);
    let sample = Position3D::new(0.0105, 0.0048, 0.0);
    assert!(within_via_neighborhood(sample, via, 0.001));
}
